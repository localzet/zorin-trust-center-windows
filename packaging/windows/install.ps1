...
$OldTasks = @(
    Get-ScheduledTask -ErrorAction SilentlyContinue |
        Where-Object { $_.TaskName -like 'ZorinTrust*' }
)
...
if ($Leftovers.Count -gt 0) {
    Write-Warning "Legacy Zorin Trust task(s) still present: $($Leftovers.TaskName -join ', '). They are not used by 0.9.0; run 6-STARTUP-DOCTOR.bat for details."
}

Write-Host 'Zorin Trust 0.9.0 installed. Silent bootstrap is running.' -ForegroundColor Green
