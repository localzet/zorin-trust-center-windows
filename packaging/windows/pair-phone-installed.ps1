...
if(-not(Test-Path -LiteralPath $Agent)){throw "Installed Zorin Host Agent not found: $Agent"}
...
& $AdbPath devices | Select-Object -Skip 1 | ForEach-Object {
  ...
  if($f.Count-ge2 -and $f[1]-eq'device'){$devices+=$f[0]}
}
if($devices.Count-ne1){throw "Pairing expects exactly one authorized adb device; found $($devices.Count)."}
$serial=$devices[0]

$TaskName='ZorinTrustHostAgent';$HadTask=$false
try{
  Import-Module ScheduledTasks -ErrorAction Stop
  $Task=Get-ScheduledTask -TaskName $TaskName -ErrorAction SilentlyContinue
  if($Task){$HadTask=$true;Stop-ScheduledTask -TaskName $TaskName -ErrorAction SilentlyContinue}
}catch{}
$pids=@()
try{$pids=@(Get-NetTCPConnection -LocalAddress '127.0.0.1' -LocalPort 47472 -State Listen -ErrorAction Stop|Select-Object -ExpandProperty OwningProcess -Unique)}catch{}
if($pids.Count){
...
try{
  ...
  Remove-Item (Join-Path $StateDir 'owner-mode.json') -Force -ErrorAction SilentlyContinue
  if($HadTask){
    try{Start-ScheduledTask -TaskName $TaskName -ErrorAction Stop;Write-Host 'Restored Zorin Host Agent autostart.' -ForegroundColor Green}
